...
| **Crypto VCP Monitor** | 暗号資産主要銘柄（BTC/ETH/SOL/BNB）向けにMinerviniのVolatility Contraction Patternの閾値を較正。APIキー不要のBinance日足データで、固定46銘柄ユニバースに対する処理群/対照群バックテストを実行し、サンプルサイズ基準をクリアするパラメータセットを報告。現在時点の検出機能はまだ無く、今後の課題 | <span class="badge badge-free">API不要</span> |
